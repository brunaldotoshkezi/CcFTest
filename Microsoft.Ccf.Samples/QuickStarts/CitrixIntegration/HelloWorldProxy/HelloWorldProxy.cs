...
{
	public class HelloWorldProxy : CitrixApplicationHostedControl
	{
		public HelloWorldProxy( int appID, string appName, string initString ): base(appID, appName, initString)
		{
#if DEBUG
			Logging.Debug = true;
#endif
			AgentCredentialRequest += delegate
			{
				...
				Logging.Trace("HelloWorldProxy: AgentCredentialRequest event handled");
			};
			...
			FirstHeartbeatReceived += delegate
			{
				...
				Logging.Trace("HelloWorldProxy: FirstHeartbeatReceived event handled");
			};
		}

		protected override string ICAFileName
		{
			get { return @"X:\CCFCITRIX\HelloWorldStub.ica"; }
		}
